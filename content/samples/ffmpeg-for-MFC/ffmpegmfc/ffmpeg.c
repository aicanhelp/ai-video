...
const char program_name[] = "ffmfc_ffmpeg";
                            ...
                            AVSubtitle *sub)
{
    ...
    for (i = 0; i < nb; i++) {
		...
		sub->pts               += av_rescale_q(sub->start_display_time, _AVRational( 1, 1000 ), AV_TIME_BASE_Q);
        ...
        pkt.duration = av_rescale_q(sub->end_display_time, _AVRational( 1, 1000 ), ost->st->time_base);
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
	...
	//设置进度（以时间为基准）
	//此处提取当前处理的时间
	ffmfcctx->media_cur_time=secs;
	ffmfcctx->progress=100*ffmfcctx->media_cur_time/ffmfcctx->media_duration;
	SendMessage(hwnddlg,WM_SETPROGRESS,IDD_FFMPEGMFC_DIALOG,0);
	//-------------------------
	//
    us = pts % AV_TIME_BASE;
    mins = secs / 60;
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (decoded_frame->pts != AV_NOPTS_VALUE)
        decoded_frame->pts = av_rescale_q(decoded_frame->pts,
                                          decoded_frame_tb,
                                          _AVRational(1, ist->st->codec->sample_rate));
    for (i = 0; i < ist->nb_filters; i++)
...
static int transcode_init(void)
{
	...
	//为设置视频进度使用
	for(i=0;i<nb_input_files;i++){
		ffmfcctx->media_duration=(*input_files[i]).ctx->duration/AV_TIME_BASE;
	}
	//
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy) {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (!codec->sample_aspect_ratio.num) {
                    ...
                    ost->st->sample_aspect_ratio =
						...
						ist->st->codec->sample_aspect_ratio : _AVRational(0, 1);
                }
            ...
            if (codec->codec_type == AVMEDIA_TYPE_VIDEO) {
                ...
                if (ist && !ost->frame_rate.num)
                    ost->frame_rate = ist->st->r_frame_rate.num ? ist->st->r_frame_rate : _AVRational(25, 1);
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                break;
            case AVMEDIA_TYPE_SUBTITLE:
                codec->time_base = _AVRational(1, 1000);
                if (!codec->width) {
...
int ffmfc_ffmpeg(void *param,int argc, char **argv)
{
	...
	hwndbutton=GetDlgItem(hwnddlg, IDC_PAUSE);
	//SendMessage
	//通过WinAPI可以调用主对话框中的函数
	//参数的意义：1.控件句柄（主对话框中this->GetDlgItem(ID_PAUSE)->GetSafeHwnd()）
	//2.消息类型
	//3.接收消息的控件ID
	//4.参数（可以没有）
	//SendMessage(hwndbutton,WM_LBUTTONDOWN,ID_PAUSE,0);
	//SendMessage(hwndbutton,WM_LBUTTONUP,ID_PAUSE,0);
	//PostMessage((HWND)dlg,WM_LBUTTONDOWN,1002,0);
	//PostMessage((HWND)dlg,WM_LBUTTONUP,1002,0);
	...
	//自定义消息
	//可以用于设置Progress
	//SendMessage(hwnddlg,WM_SETPROGRESS,IDD_VIDEOZIP,0);
    ...
    /* parse options */
	//分析选项
    if(parse_options(&o, argc, argv, options, opt_output_file)==-1){
		return -1;
	}
