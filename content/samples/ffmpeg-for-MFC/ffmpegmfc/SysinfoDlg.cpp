...
IMPLEMENT_DYNAMIC(Sysinfosubof, CDialogEx)

	Sysinfosubof::Sysinfosubof(CWnd* pParent /*=NULL*/)
	: CDialogEx(Sysinfosubof::IDD, pParent)
{

}

Sysinfosubof::~Sysinfosubof()
{
}

void Sysinfosubof::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_SYSINFO_OF, m_sysinfoof);
}


BEGIN_MESSAGE_MAP(Sysinfosubof, CDialogEx)
END_MESSAGE_MAP()
...
BOOL SysinfoDlg::OnInitDialog(){
	...
	sysinfosubof.Create(IDD_SYSINFOSUB_OF,&m_sysinfotab);
	...
	pDialog[2]=&sysinfosubof;
	pDialog[3]=&sysinfosubvc;
	pDialog[4]=&sysinfosubac;
	pDialog[5]=&sysinfosuboc;
	...
	resloader.LoadString(IDS_SYSINFO_OF);
	...
	m_sysinfotab.InsertItem(5,resloader);
	...
	//--------------------------
	sysinfosubof.m_sysinfoof.ModifyStyle(0,LVS_SINGLESEL|LVS_REPORT|LVS_SHOWSELALWAYS);
	sysinfosubof.m_sysinfoof.SetExtendedStyle(dwExStyle);

	resloader.LoadString(IDS_SYSINFO_COMMON_NUM);
	sysinfosubof.m_sysinfoof.InsertColumn(0,resloader,LVCFMT_CENTER,40,0);
	resloader.LoadString(IDS_SYSINFO_COMMON_NAME);
	sysinfosubof.m_sysinfoof.InsertColumn(1,resloader,LVCFMT_CENTER,60,0);
	resloader.LoadString(IDS_SYSINFO_COMMON_FULLNAME);
	sysinfosubof.m_sysinfoof.InsertColumn(2,resloader,LVCFMT_CENTER,200,0);
	resloader.LoadString(IDS_SYSINFO_IF_EXT);
	sysinfosubof.m_sysinfoof.InsertColumn(3,resloader,LVCFMT_CENTER,60,0);
	resloader.LoadString(IDS_SYSINFO_COMMON_PRIVSIZE);
	sysinfosubof.m_sysinfoof.InsertColumn(4,resloader,LVCFMT_CENTER,90,0);
...
void SysinfoDlg::GetSysinfo(){
	...
	si.first_of= av_oformat_next(NULL);
	si.first_c=av_codec_next(NULL);

	AVInputFormat *if_temp=si.first_if;
	AVOutputFormat *of_temp=si.first_of;
	...
	while(of_temp!=NULL){
		...
		long_name.Format(_T("%S"),of_temp->long_name);
		extensions.Format(_T("%S"),of_temp->extensions);
#else
		name.Format(_T("%s"),of_temp->name);
		long_name.Format(_T("%s"),of_temp->long_name);
		extensions.Format(_T("%s"),of_temp->extensions);
#endif
		priv_data_size.Format(_T("%d"),of_temp->priv_data_size);
		f_index.Format(_T("%d"),of_index);
		//获取当前记录条数
		nIndex=sysinfosubof.m_sysinfoof.GetItemCount();
		//“行”数据结构
		LV_ITEM lvitem;
		lvitem.mask=LVIF_TEXT;
		lvitem.iItem=nIndex;
		...
		lvitem.pszText=f_index.GetBuffer();
		//------------------------
		sysinfosubof.m_sysinfoof.InsertItem(&lvitem);
		sysinfosubof.m_sysinfoof.SetItemText(nIndex,1,name);
		sysinfosubof.m_sysinfoof.SetItemText(nIndex,2,long_name);
		sysinfosubof.m_sysinfoof.SetItemText(nIndex,3,extensions);
		sysinfosubof.m_sysinfoof.SetItemText(nIndex,4,priv_data_size);
